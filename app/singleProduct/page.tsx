...
const singleProduct = ({ searchParams }: {
  ...
  return (
    <div className='bg-orange-200 flex flex-col lg:flex-row'>
        ...
        <div className="max-w-6xl mx-auto w-full p-6 pt-32 pb-16">
            <div className="flex flex-col lg:flex-row gap-8 border border-orange-400 bg-orange-100 px-6 md:px-10 py-8 rounded-lg shadow-xl">
                ...
                <div className="flex-1 space-y-4 px-4 lg:px-10 py-6">
                    <div className="space-y-2 pt-7 pb-5">
                        <h1 className="text-[28px] md:text-[32px] mb-5 text-orange-700 font-bold">{searchParams.title}</h1>
                        <div className="flex justify-between">
                            <div className='text-black text-[20px] md:text-[22px] font-semibold mb-3 flex gap-3'>
                                <p>Price: ${searchParams.price} </p>
                                <p> | </p>
                                <p className='text-red-500 font-bold'> Discount: {searchParams.discountPercentage}%</p>
                            </div>
                        </div>
                        <div>
            ...
            <div className="flex flex-col lg:flex-row rounded-lg my-4 gap-3">
                <div className="pt-5 border border-orange-400 bg-orange-100 px-4 lg:px-10 pb-8 rounded-lg shadow-xl w-full lg:w-[560px]">
                    <h2 className="text-center text-3xl text-orange-700 font-semibold mb-12">Product Details</h2>
                    <div className="text-black py-3 border-gray-300 border-t flex justify-between font-semibold">
                        <div>Availability:</div>
                        <div>{searchParams.availabilityStatus}</div>
                    </div>
                    <div className="text-black border-t py-3 border-gray-300 flex justify-between font-semibold">
                        <div>Warranty: </div>
                        <div>{searchParams.warrantyInformation}</div>
                    </div>
                    <div className="text-black border-t py-3 border-gray-300 flex justify-between font-semibold">
                        <div>Dimensions:</div>
                        <div>{dimensions.height} x {dimensions.width} x {dimensions.depth} cm</div>
                    </div>
                    <div className="text-black border-t py-3 border-gray-300 flex justify-between font-semibold">
                        <div>Weight: </div>
                        <div>{searchParams.weight} kg</div>
                    </div>
                    <div className="text-black border-t py-3 border-gray-300 flex justify-between font-semibold">
                        <div>Stock: </div>
                        <div>{searchParams.stock}</div>
                    </div>
                    <div className="text-black border-t py-3 border-gray-300 flex justify-between font-semibold">
                        <div>Min Order:</div>
                        <div>{searchParams.minimumOrderQuantity}</div>
                    </div>
                    <div className="text-black border-y py-3 border-gray-300 flex justify-between font-semibold">
                        <div>Return Policy:</div>
                        <div>{searchParams.returnPolicy}</div>
                    </div>
                </div>

                <div className="pt-5 border w-full lg:w-[560px] border-orange-400 bg-orange-100 px-4 lg:px-10 pb-8 rounded-lg shadow-xl">
